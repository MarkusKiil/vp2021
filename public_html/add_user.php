    ...
    if($_SERVER["REQUEST_METHOD"] === "POST"){
		if(isset($_POST["user_data_submit"])){
			...
			if(isset($_POST["first_name_input"]) and !empty($_POST["first_name_input"])){
				...
				if(empty($firstname)){
					$firstname_error = "Palun sisestsa oma eesnimi!";
				}
			}else{
			...
			if(isset($_POST["surname_input"]) and !empty($_POST["surname_input"])){
				...
				if(empty($surname)){
					$surname_error = "Palun sisestsa oma perekonnanimi!";
				}
			}else{
			...
			if(isset($_POST["gender_input"]) and !empty($_POST["gender_input"])){
				...
				if($gender != 1 and $gender != 2){
					$gender_error = "Palun sisestsa oma perekonnanimi!";
				}
			}else{
				$surnamee_error = "Palun sisesta oma perekonnanimi!";
			}
			...
			if(isset($_POST["birth_day_input"]) and !empty($_POST["birth_day_input"])){
				...
				if($birth_day < 1 or $birth_day > 31){
					$birth_day_error = "Palun sisestsa sünnipäev!";
				}
			}else{
				$birth_day_error = "Palun sisesta oma sünnipäev!";	
			}
			...
			if(isset($_POST["birth_month_input"]) and !empty($_POST["birth_month_input"])){
				...
				if($birth_month < 1 or $birth_month > 12){
					$birth_month_error = "Palun sisestsa sünnikuu!";
				}
			}else{
				$birth_month_error = "Palun sisesta oma sünnikuu!";	
			}
			...
			if(isset($_POST["birth_year_input"]) and !empty($_POST["birth_year_input"])){
				...
				if($birth_year < date("Y") - 110 or $birth_year > date("Y") - 13){
					$birth_year_error = "Palun sisestsa sünni aasta!";
				}
			}else{
				$birth_year_error = "Palun sisesta oma sünni aasta!";	
			}
			//kuupäeva valideerimine
			if(empty($birth_day_error) and empty($birth_month_error) and empty($birth_year_error)){
				if(checkdate($birth_month, $birth_day, $birth_year)){
					$tempdate = new DateTime($birth_year ."-" .$birth_month ."-" .$birth_day);
					$birth_date = $tempdate->format("Y-m-d");
				}else{
			...
			if(isset($_POST["email_input"]) and !empty($_POST["email_input"])){
				...
				if(empty($email)){
					$email_error = "Palun sisestsa e-posti aadress!";
				}
			}else{
			...
			if(isset($_POST["password_input"]) and !empty($_POST["password_input"])){
				if(strlen($_POST["password_input"]) < 8){
					$password_error = "Palun sisestsa pikem salasõna!";
				}
			}else{
			...
			//kui kõik kombes salvetame uue kasutaja
			if(empty($firstname_error) and empty($surname_error) and empty($birth_date_error) and empty($gender_error) and empty($email_error) and empty($password_error) and empty($confirm_password_error)){
